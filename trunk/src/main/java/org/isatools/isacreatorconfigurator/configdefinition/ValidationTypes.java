...
/**
 * Enum class to define the types of validation allowed for numeric validation
 *
 * @author eamonnmaguire
 * @date Aug 26, 2009
 */
public enum ValidationTypes {
    BETWEEN("between"), EQUAL_TO("equal to"), LESS_THAN("less than"), MORE_THAN("more than"),
    LESS_EQUAL_TO("less than or equal to"), MORE_EQUAL_TO("greater than or equal to");

    private String description;

    ValidationTypes(String description) {
    ...
    public static ValidationTypes resolveValidationType(String validation) {
        for (ValidationTypes vt : values()) {
            if (vt.getDescription().equals(validation)) {
                return vt;
            }
        }
