...
package org.isatools.isacreatorconfigurator.ontologyconfigurationtool.io;
...
public class Utils {
    ...
    // cot -> cached ontology tree
    private static final String EXTENSION = ".cot";

    /**
     * Utility method to save the Ontology tree as a method of caching the resolved tree since loading and classifying
     * ontologies with numerous and complex axioms takes a lot of time!
     *
     * @param ontologyVersion - Unique ontology version number to not only determine which ontology to use, but which version also (BioPortal specific)
     * @param tree            - The root node of the tree to be saved!
     * @throws java.io.IOException - thrown on event of problem encountered during saving
     */
    public static void saveOntologyTree(String ontologyVersion, DefaultMutableTreeNode tree) throws IOException {
        FileOutputStream fos = null;
        ObjectOutputStream oos = null;
        try {
            fos = new FileOutputStream(BioPortalClient.DOWNLOAD_ONTOLOGY_LOC + ontologyVersion + EXTENSION);
    ...
    public static DefaultMutableTreeNode loadOntologyTree(String ontologyVersion) throws IOException, ClassNotFoundException {

        File f = new File(BioPortalClient.DOWNLOAD_ONTOLOGY_LOC + ontologyVersion + EXTENSION);
