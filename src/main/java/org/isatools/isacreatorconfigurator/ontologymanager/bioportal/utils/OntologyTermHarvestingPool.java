...
package org.isatools.isacreatorconfigurator.ontologymanager.bioportal.utils;
...
/**
 * A service which asks for the extra information for an ontology term
 *
...
public class OntologyTermHarvestingPool implements Runnable {
    ...
    private Map<String, BioPortalOntology> toProcess;
    ...
    public void executeRequests(String ontology, Map<String, BioPortalOntology> toProcess) {
    ...
    public void run() {
        for (String accession : toProcess.keySet()) {
            threadPool.execute(new OntologyHandler(accession));
        }
    }

    class OntologyHandler implements Runnable {
        private String termAccession;

        OntologyHandler(String termAccession) {
            this.termAccession = termAccession;
        }

        public void run() {
            BioPortalOntology term = client.getTermInformation(termAccession, ontology);
            addToResults(termAccession, term);
        }
    }

    private synchronized void addToResults(String termAccession, BioPortalOntology ontologyTerm) {
        toProcess.put(termAccession, ontologyTerm);
    }
}
