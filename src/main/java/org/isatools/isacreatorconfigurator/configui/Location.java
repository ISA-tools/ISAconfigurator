...
public enum Location {
    INVESTIGATION("Investigation file", "Investigation File"),
    STUDY_SAMPLE("Study sample", "Study Sample File"),
    ASSAY("Assay", "Assay File");

    private String type;
    private String alias;

    Location(String type, String alias) {
        this.type = type;
        this.alias = alias;
    }
    ...
    public static Location resolveLocationIdentifier(String location) {
        for (Location candidateLocation : values()) {
            if (candidateLocation.getAlias().equalsIgnoreCase(location)
                    || candidateLocation.getType().equalsIgnoreCase(location)) {
